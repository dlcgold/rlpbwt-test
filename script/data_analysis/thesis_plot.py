...
L1 = ax2.plot(x_maked, y_maked, label="PBWT", marker="o")[0]
...
L2 = ax2.plot(x_makep, y_makep, label="MAP-BV + THR-BV + RA-BV + PERM + PHI", marker="d")[0]
...
L3 = ax2.plot(x_maket, y_maket, label="MAP-BV + THR-BV + RA-SLP + PERM + PHI", marker="s")[0]
...
L4 = ax2.plot(x_makes, y_makes, label="MAP-BV + LCE + PERM + PHI", marker="p")[0]

## make bitvector panel + threshold
tmp_df = df_makes[df_makes["variant"] == "panel extended raw"].reset_index(drop=True)
x_makep, y_makep = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "wall_clock"])) for i in range(len(tmp_df))])
#plt.scatter(x_makep, y_makep)
L6 = ax2.plot(x_makep, y_makep, label="MAP-INT + THR-INT + RA-BV + PERM + PHI", marker="1")[0]

## make slp+threshold
tmp_df = df_makes[df_makes["variant"] == "slp_thr_raw extended"].reset_index(drop=True)
x_maket, y_maket = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "wall_clock"])) for i in range(len(tmp_df))])
#plt.scatter(x_maket, y_maket)
L7 = ax2.plot(x_maket, y_maket, label="MAP-INT + THR-INT + RA-SLP + PERM + PHI", marker="2")[0]

## make slp+lce
tmp_df = df_makes[df_makes["variant"] == "slp_no_thr_raw extended"].reset_index(drop=True)
x_makes, y_makes = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "wall_clock"])) for i in range(len(tmp_df))])
#plt.scatter(x_makes, y_makes)
L8 = ax2.plot(x_makes, y_makes, label="MAP-INT + LCE + PERM + PHI", marker="3")[0]
...
ax2.set_title(f"(b)\nConstruction time", fontweight="bold")
ax2.set_xticks(x_makes)
ax2.set_xlabel("#sites", fontweight="bold")
...
#plt.scatter(x_maked, y_maked)
L1 = ax1.plot(x_maked, y_maked, label="PBWT", marker="o")[0]
...
#plt.scatter(x_makep, y_makep)
L2 = ax1.plot(x_makep, y_makep, label="MAP-BV + THR-BV + RA-BV + PERM + PHI", marker="d")[0]

## make slp+threshold
tmp_df = df_makes[df_makes["variant"] == "slp_thr extended"].reset_index(drop=True)
x_maket, y_maket = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "max_mem"] * to_gb)) for i in range(len(tmp_df))])
#plt.scatter(x_maket, y_maket)
L3 = ax1.plot(x_maket, y_maket, label="MAP-BV + THR-BV + RA-SLP + PERM + PHI", marker="s")[0]

## make slp+lce
tmp_df = df_makes[df_makes["variant"] == "slp_no_thr extended"].reset_index(drop=True)
x_makes, y_makes = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "max_mem"] * to_gb)) for i in range(len(tmp_df))])
#plt.scatter(x_makes, y_makes)
L4 = ax1.plot(x_makes, y_makes, label="MAP-BV + LCE + PERM + PHI", marker="p")[0]

## make bitvector panel + threshold
tmp_df = df_makes[df_makes["variant"] == "panel extended raw"].reset_index(drop=True)
x_makep, y_makep = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "max_mem"] * to_gb)) for i in range(len(tmp_df))])
#plt.scatter(x_makep, y_makep)
L6 = ax1.plot(x_makep, y_makep, label="MAP-INT + THR-INT + RA-BV + PERM + PHI", marker="1")[0]

## make slp+threshold
tmp_df = df_makes[df_makes["variant"] == "slp_thr_raw extended"].reset_index(drop=True)
x_maket, y_maket = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "max_mem"] * to_gb)) for i in range(len(tmp_df))])
#plt.scatter(x_maket, y_maket)
L7 = ax1.plot(x_maket, y_maket, label="MAP-INT + THR-INT + RA-SLP + PERM + PHI", marker="2")[0]

## make slp+lce
tmp_df = df_makes[df_makes["variant"] == "slp_no_thr_raw extended"].reset_index(drop=True)
x_makes, y_makes = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "max_mem"] * to_gb)) for i in range(len(tmp_df))])
#plt.scatter(x_makes, y_makes)
L8 = ax1.plot(x_makes, y_makes, label="MAP-INT + LCE + PERM + PHI", marker="3")[0]
...
ax1.set_title(f"(a)\nConstruction max memory", fontweight="bold")
...
plt.tight_layout()
line_labels = ["MAP-BV + THR-BV + RA-BV + PERM + PHI", "MAP-BV + THR-BV + RA-SLP + PERM + PHI", "MAP-BV + LCE + PERM + PHI", "MAP-INT + THR-INT + RA-BV + PERM + PHI", "MAP-INT + THR-INT + RA-SLP + PERM + PHI", "MAP-INT + LCE + PERM + PHI", "MAP-INT + RLCP", "MAP-BV + RLCP", "PBWT"]
fig.legend(handles= [L2, L3, L4, L6, L7, L8, L9, L10,L1],     # The line objects
...
plt.savefig(f"../thesis_figures/make_time_mem_paper.pdf", dpi=500, bbox_inches='tight')
...
L1 = ax2.plot(x_makei, y_makei, label="PBWT MatchIndexed", marker="o")[0]
...
#plt.scatter(x_makei, y_makei)
L1 = ax1.plot(x_makei, y_makei, label="PBWT MatchIndexed", marker="o")[0]
...
line_labels = ["PBWT MatchIndexed", "PBWT MatchDynamic", "MAP-BV + THR-BV + RA-BV + PERM + PHI", "MAP-BV + THR-BV + RA-SLP + PERM + PHI", "MAP-BV + LCE + PERM + PHI", "MAP-INT + THR-INT + RA-BV + PERM + PHI",  "MAP-INT + THR-INT + RA-SLP + PERM + PHI","MAP-INT + LCE + PERM + PHI", "MAP-INT + RLCP", "MAP-BV + RLCP"]
fig.legend(handles= [L1, L5, L2, L3, L4, L6, L7, L8, L9, L10],     # The line objects
           labels=line_labels,   # The labels for each line
           loc="lower center",   # Position of legend
           #borderaxespad=0.1,    # Small spacing around legend box
...
plt.savefig(f"../thesis_figures/exe_time_mem_paper.pdf", dpi=500, bbox_inches='tight')
...
L11 = ax3.plot(x_makei, y_makei, color='#5e81ac')[0]
...
L21 = ax3.plot(x_makep, y_makep, color='#88c0d0')[0]
...
L31 = ax3.plot(x_maket, y_maket, color='#a3be8c')[0]
...
L41 = ax3.plot(x_makes, y_makes, color='#bf616a')[0]
...
x_makep, y_makep, e_makep = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "mean"]), float(tmp_df.loc[i, "stdev"])) for i in range(len(tmp_df))])
#plt.scatter(x_makep, y_makep, marker="d", label="MAP-BV + THR-BV + RA-BV + PERM + PHI")
L6 = ax3.errorbar(x_makep, y_makep, yerr=e_makep, label="MAP-INT + THR-INT + RA-BV + PERM + PHI", fmt="1", capsize=2, color='#b48ead')[0]
L61 = ax3.plot(x_makep, y_makep, color='#b48ead')[0]

## run slp+threshold
tmp_df = df_means[df_means["variant"] == "slp_thr_raw extended"].reset_index(drop=True)
x_maket, y_maket, e_maket = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "mean"]), float(tmp_df.loc[i, "stdev"])) for i in range(len(tmp_df))])
#plt.scatter(x_maket, y_maket, marker="s", label="MAP-BV + THR-BV + RA-SLP + PERM + PHI")
L7 = ax3.errorbar(x_maket, y_maket, yerr=e_maket, label="MAP-INT + THR-INT + RA-SLP + PERM + PHI", fmt="2", capsize=2, color='#4c566a')[0]
L71 = ax3.errorbar(x_maket, y_maket, color='#4c566a')[0]

## run slp+lce
tmp_df = df_means[df_means["variant"] == "slp_no_thr_raw extended"].reset_index(drop=True)
x_makes, y_makes, e_makes = zip(*[(float(tmp_df.loc[i, "sites"]),float(tmp_df.loc[i, "mean"]), float(tmp_df.loc[i, "stdev"])) for i in range(len(tmp_df))])
#plt.scatter(x_makes, y_makes, marker="p", label="MAP-BV + LCE + PERM + PHI")
L8 = ax3.errorbar(x_makes, y_makes, yerr=e_makes, label="MAP-INT + LCE + PERM + PHI", fmt="3", capsize=2, color='#d08770')[0]
L81 = ax3.errorbar(x_makes, y_makes, color='#d08770')[0]
...
ax3.set_xlabel("#sites", fontweight="bold")
...
line_labels = ["PBWT MatchIndexed", "PBWT MatchDynamic", "MAP-BV + THR-BV + RA-BV + PERM + PHI", "MAP-BV + THR-BV + RA-SLP + PERM + PHI", "MAP-BV + LCE + PERM + PHI", "MAP-INT + THR-INT + RA-BV + PERM + PHI", "MAP-INT + THR-INT + RA-SLP + PERM + PHI", "MAP-INT + LCE + PERM + PHI"]
fig.legend(handles= [L1, L5, L2, L3, L4, L6, L7, L8],     # The line objects
           labels=line_labels,   # The labels for each line
...
plt.savefig(f"../thesis_figures/exe_time_single_paper.pdf", dpi=500, bbox_inches='tight')
...
plt.savefig(f"../thesis_figures/prep_mem_time.pdf", dpi=500, bbox_inches='tight')